...
uint32_t extractParity(QString val, int num)
{
    ...
    QByteArray hex = QByteArray::fromHex(val.replace('!', "").toUtf8());
	for(int i = 0; i < num; ++i)
...
{
    QByteArray hex = QByteArray::fromHex(val.replace('!', "").replace(' ',"").toUtf8());
	int lsb = 0;
	for(int i = 0; i < num; ++i)
		lsb = lsb << 1 | (hex[i] & 1);
...
{
    uint64_t key = QByteArray(strRevState->text().replace(' ',"").toUtf8()).toULongLong(0, 16);
...
{
    uint64_t key = QByteArray(strRevState->text().replace(' ',"").toUtf8()).toULongLong(0, 16);
    QByteArray ba = QByteArray::fromHex(strEncrypted->text().replace(' ',"").replace('!',"").toUtf8());
	QString dec = "";
	uint32_t ks = 0;
	struct Crypto1State *s = crypto1_create(key);
...
{
    uint32_t filter = strtoul(strParities->text().replace(' ', "").toUtf8(), 0, 2);
...
bool Crapto1Gui::doTryOne(uint32_t nonce)
{
    ...
    uint32_t encrc = strtoul(encRCRR.left(8).toUtf8(),0,16);
    uint32_t encrr = strtoul(encRCRR.right(8).toUtf8(),0,16);
...
void Crapto1Gui::doTryAll()
{
	...
	{
        uint32_t nonce = strtoul(listWidget->item(i)->text().toUtf8(), 0, 16);
